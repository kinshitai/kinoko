...
The HTTP boundary between `kinoko run` and `kinoko serve`. The local daemon (`run`) never opens the index DB directly — all reads and writes go through `serve`'s HTTP API via the `serverclient` package. This enables multiple `run` instances to share one `serve` without SQLite lock contention. See also: **serverclient**, **Index DB**, **Queue DB**.
...
**Index DB**
The SQLite database owned by `kinoko serve`. Stores skills, embeddings, injection events, sessions, and human review samples. This is the derived cache — it can be rebuilt from git at any time. Only `serve` opens this database; `run` accesses it indirectly through the HTTP API. See also: **Queue DB**, **Client/Server Boundary**.
...
**Queue DB**
The SQLite database owned by `kinoko run`. Stores the local job queue: pending session logs awaiting extraction, claim state, retry counts, and backpressure metadata. Separate from the index DB to avoid cross-process locking. Implemented by `internal/queue/Store`. See also: **Index DB**, **Client/Server Boundary**.
...
**serverclient**
The HTTP client package (`internal/serverclient/`) used by `kinoko run` to communicate with `kinoko serve`. Provides typed Go clients for all server API endpoints: search, embed, novelty, decay, injection events, sessions, and skill commit. This is the client side of the client/server boundary. See also: **Client/Server Boundary**.
...
**SkillStore**
The main storage interface (`storage.SkillStore`): `Put`, `Get`, `GetLatestByName`, `Query`, `UpdateUsage`, `UpdateDecay`, `ListByDecay`. Implemented by `SQLiteStore` on the server side (index DB). On the client side (`kinoko run`), operations go through the `serverclient` package over HTTP.
