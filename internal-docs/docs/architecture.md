...
This section explains **why** the architecture is shaped the way it is, linking key decisions to research findings (primarily [brief-012](../briefs/brief-012.md) and SkillsBench evaluations).

### Why the client/server split?

The split was introduced in T1–T8 to separate concerns along a natural boundary: **extraction needs local session access** (sessions are files on the developer's machine), while the **server is a shared knowledge store** (multiple developers share one skill library). This is not premature abstraction — it enables: (a) multiple developers sharing one skill library without filesystem coupling, (b) keeping LLM API keys and extraction costs on the client side, (c) future multi-server topologies and federation.

### Why extraction is client-side

**Data locality.** Session logs are local files generated by the developer's agent. Shipping raw session content to the server would add latency, bandwidth cost, and privacy concerns. Keeping extraction client-side means the server never sees raw session data — only extracted, structured skills arrive via git push. Additionally, clients can use different LLM providers without server config changes, and the server stays fast and deterministic.

### Why max 3 skills injected (MaxSkills = 3)

This is empirically derived from SkillsBench evaluations (brief-012 §3). The relationship between injected skill count and downstream agent performance is **non-monotonic**:

- **2–3 skills:** +18.6 percentage points (optimal range)
- **4+ skills:** +5.9 percentage points (degrades by 12.7pp vs. the 2–3 range)

More skills create context noise. The cap at 3 maximizes signal-to-noise ratio.

### Why compact skill format over comprehensive

SkillsBench found that **compact skills outperform comprehensive ones** by a significant margin:

- Compact format: +18.8pp improvement
- Comprehensive format: −2.9pp (actually *hurts* performance)

This is why Stage 3's content truncation at 100KB is a feature, not a limitation — and why the extraction pipeline favors concise, actionable procedures over exhaustive documentation.

### Why the 7-dimension scoring weights

The dimensional weights (§8) are calibrated to the primary failure mode identified in brief-012: **vague, non-actionable procedures**. Problem Specificity (0.15) and Solution Completeness (0.20) receive the highest combined weight (0.35) because the #1 failure mode in auto-extracted skills is lack of specificity. Technical Accuracy (0.20) is weighted equally to Solution Completeness because inaccurate skills are worse than no skills.

The minimum thresholds (≥3 on Problem Specificity, Solution Completeness, and Technical Accuracy) exist because brief-012 found that skills scoring below this floor are "vague, non-actionable procedures" — the precision gap.

---

## 11. Roadmap & Future

Brief notes on planned architectural changes. These are **not yet implemented** — do not design against them.

- **Hook integration.** §10 caveat #3 (ingest wiring gap) will be addressed with a client-side hook pattern: agent → `kinoko ingest` triggered automatically on session end. The server-side post-receive hook pattern already exists; the client-side equivalent is pending.
- **Multi-server layering.** The current architecture assumes one `kinoko serve` + N `kinoko run` instances. Future work includes: library federation (multiple serves sharing skills), hierarchical skill libraries (team → org → public), and read replicas for large deployments.
- **Authentication.** Currently unauthenticated (localhost assumption). The boundary is designed to make auth easy to add — all communication goes through two chokepoints: HTTP API and SSH git push.
- **Embedding model migration.** §4.2 currently uses `text-embedding-3-small`. The schema stores `model_name` per embedding for forward-compatibility. A re-indexing plan for model switches is pending.

---

## 12. Known Architectural Caveats
