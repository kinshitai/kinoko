...
The pipeline described above runs across two cooperating processes:

- **`kinoko serve`** owns shared infrastructure: the git server, discovery API, and the **index DB** (skills, embeddings, injection events). Think of it as the library building.
- **`kinoko run`** owns local work: the **queue DB** (pending session logs), worker pool, extraction pipeline, scheduler, and injection. Think of it as the research team that uses the library.

They communicate over HTTP via the `serverclient` package. This split means multiple `run` instances (different machines, different teams) can share one `serve` — the library scales independently of the researchers.

## Further Reading

- [Architecture](./architecture.md) — Technical component descriptions, data flow, and API boundaries (on `docs/architecture-rewrite` branch)
