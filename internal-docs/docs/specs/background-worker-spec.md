...
> **⚠️ Partial Staleness Note (2026-02-17):** This spec is largely implemented and accurate. Key changes since writing:
> - The worker pool and queue now run in `kinoko run`, not `kinoko serve`. The queue uses its own **queue DB** (SQLite), separate from the index DB in `serve`.
> - **§5 CLI** — `kinoko serve` no longer starts the worker pool. `kinoko run` starts workers + scheduler. The standalone `kinoko worker` command was not implemented; `kinoko run` covers that role.
> - Workers communicate with `serve` via the `serverclient` HTTP client for skill commits, search, embedding, and decay — not direct DB access.
