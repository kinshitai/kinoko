...
> **⚠️ Partial Staleness Note (2026-02-17):** This spec was written before the `serve`/`run` split. The data model, interfaces, evaluation framework, and cost model remain accurate. Key changes:
> - **Storage** now lives behind `kinoko serve`'s HTTP API. `kinoko run` accesses skills, embeddings, and sessions via `serverclient`, not direct SQLite calls.
> - **§2.3 Trigger Map** — "Session ends" triggers `queue.Enqueue()` on the local queue DB (fast path), not direct pipeline execution. Workers in `kinoko run` process the queue asynchronously. See [background-worker-spec.md](background-worker-spec.md).
> - **§5.3 Storage Failure** — SQLite failures now affect two databases: the index DB (serve) and queue DB (run). Network failures between run↔serve are a new failure mode handled by `serverclient` with retries.
> - **§6 Implementation Plan** — Phase 8 ("CLI Integration") is superseded; extraction runs in `kinoko run`'s worker pool, not as a hook in `kinoko serve`.
