...
- **Registration API** — `POST /api/v1/register` for client SSH key registration (optionally token-secured)
...
- Creates `~/.kinoko/` with config, cache dir, and client SSH key (`id_ed25519`)
- With `--connect &lt;url&gt;`, configures as a client of a remote server and auto-registers the SSH key
- With `--connect` and `--token`, authenticates key registration (required when the server has `registrationToken` set)
...
| Port+2 (23233) | Kinoko API (discover, embed, ingest) | HTTP |
...
The `init --connect` command registers the client's SSH key with the server automatically. If the server requires token-based registration (via `server.registrationToken`), pass `--token`:

```bash
kinoko init --connect server-host:23231 --token <registration-token>
```

Each developer's `run` daemon extracts knowledge locally (bearing the LLM cost) and pushes to the shared server using the client SSH key. Injection pulls from the same shared pool.
